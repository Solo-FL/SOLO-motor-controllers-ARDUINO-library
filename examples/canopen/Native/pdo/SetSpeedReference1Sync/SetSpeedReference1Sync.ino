...
void setup()
{
  ...
  config.parameterName = SOLOMotorControllersCanopenNative::PdoParameterName::SPEED_REFERENCE;
...
void loop()
{
  ...
  solo->SetPdoSpeedReference(1, errorSolo);
  Serial.print("Set value: 1 ERROR: ");
  Serial.println(errorSolo);

  long getValue = solo->GetSpeedReference(errorSolo);
  ...
  getValue = solo->GetSpeedReference(errorSolo);
