...
void setup()
{
  ...
  config.parameterName = SOLOMotorControllersCanopenNative::PdoParameterName::QUADRATURE_CURRENT_IQ_FEEDBACK;
...
void loop()
{
  ...
  long getValue = solo->GetPdoQuadratureCurrentIqFeedback(errorSolo);
  ...
  // read the older value in the PDO buffer
  getValue = solo->GetPdoQuadratureCurrentIqFeedback(errorSolo);
  Serial.print("READ VALUE: ");
  Serial.print(getValue);
  Serial.print(" ERROR: ");
  Serial.println(errorSolo);
